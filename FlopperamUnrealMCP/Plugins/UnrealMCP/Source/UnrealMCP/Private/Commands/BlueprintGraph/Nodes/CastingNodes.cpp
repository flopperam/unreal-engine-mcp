...
UK2Node* FCastingNodeCreator::CreateDynamicCastNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
...
UK2Node* FCastingNodeCreator::CreateClassDynamicCastNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	if (!Graph || !Params.IsValid())
	{
		return nullptr;
	...
	UK2Node_ClassDynamicCast* ClassDynamicCastNode = NewObject<UK2Node_ClassDynamicCast>(Graph);
...
UK2Node* FCastingNodeCreator::CreateCastByteToEnumNode(UEdGraph* Graph, const TSharedPtr<FJsonObject>& Params)
{
	if (!Graph || !Params.IsValid())
	{
		return nullptr;
	...
	UK2Node_CastByteToEnum* CastByteNode = NewObject<UK2Node_CastByteToEnum>(Graph);
	if (!CastByteNode)
	{
		return nullptr;
	}
